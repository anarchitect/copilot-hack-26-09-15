...
export function UploadZone({ onUpload, maxFiles = 10, className = "" }: UploadZoneProps) {
  ...
  const uploadTimers = useRef(new Map<string, {
  ...
  useEffect(() => {
    ...
    return () => {
      timers.forEach(({ intervalId, timeoutId }) => {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    newFiles.forEach(fileObj => {
      ...
      const timeoutId = setTimeout(() => {
        clearInterval(intervalId);
        uploadTimers.current.delete(fileObj.id);
        setUploadedFiles(prev => 
          prev.map(f => 
            f.id === fileObj.id 
      ...
      uploadTimers.current.set(fileObj.id, { intervalId, timeoutId });
    });
  ...
  const removeFile = (id: string) => {
    setUploadedFiles(prev => {
      ...
      if (file) {
        ...
        objectUrls.current.delete(file.preview);
      }
