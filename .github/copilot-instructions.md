...
1. **Think Before Coding**
   - State important assumptions and surface uncertainty before implementation
   - Clarify materially different interpretations instead of choosing silently
   - Prefer the simplest viable approach and explain meaningful tradeoffs

2. **Simplicity First**
   - Write the minimum code needed to satisfy the request
   - Do not add speculative features, single-use abstractions, or unrequested configurability
   - Simplify solutions that are substantially larger than necessary

3. **Make Surgical Changes**
   - Change only what the request requires and preserve existing conventions
...
4. **Execute Toward Verifiable Goals**
   ...
   - Reproduce failures before fixing them when practical
   - Verify equivalent behavior before and after refactors
   - Continue until every success criterion is verified
