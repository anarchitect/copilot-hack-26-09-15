# Walkthrough: Customize Copilot (original guide)
...
- [ ] Know how to monitor your AI credit usage
...
## 📊 Step 1: Monitor AI Credit Usage

Most Copilot plans moved from premium requests to AI credits on June 1, 2026. Premium requests now apply only to Copilot Pro and Copilot Pro+ subscribers on eligible legacy annual plans.

### Option A: Inspect Session Usage

In IDE:

1. **Open VS Code** and ensure GitHub Copilot is active
2. **Open Copilot Chat** and submit a request
3. **Locate the context window control:** Find it in the chat input
4. **View usage:** Hover over the control to see token usage and the total AI credits consumed by the session

### Option B: Access Billing and Licensing
...
Review the usage shown for your plan. If your account remains on legacy premium request billing, the Copilot status menu in VS Code continues to show your allowance and reset date.
...
```
4. In `GalleryGrid.tsx`, select the `filteredPhotos` calculation that filters by tags and search query. Avoid selecting the Grid/List rendering blocks so each model receives the same focused task.
...
- There are three prompt files in the folder. Two are relevant to this exercise and vary in difficulty.
...
2. Type `/init`
3. Press Enter and review the proposed workspace instructions

Compare the proposal with `.github/copilot-instructions.md`. Keep the existing file for this exercise, and discuss which generated instructions would improve it.
...
- [ ] Explored AI credit monitoring and identified usage patterns
